...
function safeDateDisplay(dateStr?: string) {
  if (!dateStr) return "—";
...
export type Rule42ManagerProps = {
  regattaId: number;
  heading?: string;
};
...
export default function Rule42Manager({ regattaId, heading = "Rule 42" }: Rule42ManagerProps) {
  ...
  async function fetchRows() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Failed to load Rule 42 records.");
      setRows([]);
    } finally {
      setLoading(false);
    }
