...
export default function Step2({ data, onChange, onNext, onBack }: Step2Props) {
  ...
  return (
    <form onSubmit={handleNext} className="space-y-4">
      ...
      <input type="email" name="email" placeholder="Email" value={localData.email || ''} onChange={handleChange} className="w-full p-2 border rounded" required />
      ...
      <div className="flex justify-between mt-4">
        ...
        <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Próximo</button>
      </div>
    </form>
  )
}
