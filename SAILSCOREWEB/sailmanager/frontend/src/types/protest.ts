...
export default function useProtestPage(regattaId: number | null, token?: string) {
  ...
  // Evitar refetch da mesma classe: registo do que já foi carregado
  const fetchedClassesRef = useRef<Set<string>>(new Set());

  // Carregar entries por classe (memoizado e sem depender de entriesByClass)
  const ensureClassEntries = useCallback(async (className: string) => {
    const key = (className || '').trim();
    if (!key || !regattaId) return;
    if (fetchedClassesRef.current.has(key)) return;
    ...
    try {
      ...
      fetchedClassesRef.current.add(key);
    } catch {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setEntriesByClass({});
      fetchedClassesRef.current.clear();
      setClasses([]);

      try {
        // 1) Minhas entries desta regata
        let mine = await apiGet<EntryOption[]>(
          `/entries?mine=true&regatta_id=${regattaId}`, token
        ).catch(() => []);
        ...
        const mineForThis = (mine || []).filter(
          (e) => e.regatta_id === undefined || e.regatta_id === regattaId
        );
        ...
        const cls = await apiGet<string[]>(
          `/regattas/${regattaId}/classes`, token
        ).catch(() => []);
        ...
        if (finalClasses.length === 0) {
          const regEntries = await apiGet<EntryOption[]>(
            `/entries/by_regatta/${regattaId}`, token
          ).catch(() => []);
          finalClasses = Array.from(new Set(
            (regEntries || [])
  ...
  useEffect(() => {
    ...
    // carrega entries dessa classe (se ainda não carregado)
    void ensureClassEntries(iniClass);
  }, [initiatorEntryId, myEntries, ensureClassEntries]);
  ...
  const submit = async (): Promise<boolean> => {
    ...
    setError(null);
    try {
      if (!regattaId) return (setError('Regatta is not set.'), false);
      ...
      for (const r of respondents) {
        ...
        if (r.type === 'coach' && !r.name_text?.trim()) {
          return (setError('Enter the coach name.'), false);
        }
      }
      ...
      setError(e?.message || 'Failed to submit protest.');
      return false;
    } finally {
      setSubmitting(false);
    }
