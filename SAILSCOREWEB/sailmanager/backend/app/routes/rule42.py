...
def list_rule42(
    ...
    if class_name: q = q.filter(models.Rule42Record.class_name == class_name)
    if sail_num:   q = q.filter(models.Rule42Record.sail_num == sail_num)
    if race:       q = q.filter(models.Rule42Record.race == race)
    if group:      q = q.filter(models.Rule42Record.group == group)
    return q.order_by(models.Rule42Record.date.desc(), models.Rule42Record.id.desc()).all()

@router.post("/", response_model=schemas.Rule42Out, status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_role(["admin"]))])
...
@router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(verify_role(["admin"]))])
