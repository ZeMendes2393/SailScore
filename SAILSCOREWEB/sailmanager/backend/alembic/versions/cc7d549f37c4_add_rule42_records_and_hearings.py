...
def upgrade() -> None:
    ...
    # --- rule42_records ---
    if not has_table("rule42_records"):
        op.create_table(
            "rule42_records",
    ...
    create_index_if_not_exists("ix_rule42_records_regatta_id", "rule42_records", ["regatta_id"])

    # --- hearings ---
    if not has_table("hearings"):
        op.create_table(
            "hearings",
            sa.Column("id", sa.Integer(), primary_key=True),
            sa.Column("regatta_id", sa.Integer(), nullable=False, index=False),
    ...
    create_index_if_not_exists("ix_hearings_regatta_id", "hearings", ["regatta_id"])
    create_unique_constraint_if_not_exists(
        "uq_hearing_case_per_regatta", "hearings", ["regatta_id", "case_number"]
    )
...
def downgrade() -> None:
    ...
    if has_table("hearings"):
        op.drop_table("hearings")

    if has_index("rule42_records", "ix_rule42_records_regatta_id"):
        op.drop_index("ix_rule42_records_regatta_id", table_name="rule42_records")
    if has_table("rule42_records"):
